...
### Readiness gate
- **G1 reads scaled_confidence, NOT blended_confidence directly.** `tasks/trade_readiness_gate.py:G1_CONFIDENCE_THRESHOLD = 0.05` (normal) / `G1_FAILSAFE_CONFIDENCE_THRESHOLD = 0.10` (fail-safe). The gate compares `scaled_confidence = blended_confidence * regime_confidence` against the threshold. Reading the SKIPPED `reason="G1_blended_confidence"` string and assuming the comparison is `blended_confidence < 0.05` is wrong — the **2026-05-12 zero-trade incident** diagnostic spent real cycles on this misread. The actual chain is: a market with no series prior falls through to `_time_prior(days_to_close)` → flat weights → low `regime_confidence` → fails G4 (0.20) → fail-safe mode activates → G1 threshold raises to 0.10 → `bc * rc < 0.10` blocks the trade. The upstream binding constraint is almost always **G4**, surfaced as a G1 skip because the gate-failure-reason ordering reports G1 first.
- **Series-prior coverage is operationally load-bearing.** Markets reaching BLEND_DECISION on a series not in `analysis/regime_classifier._SERIES_PRIORS` get `_time_prior` fallback weights which produce `regime_confidence ∈ {0.063, 0.08, 0.14}` for the 3-7d / 1-3d / 7-14d buckets respectively — all below G4=0.20. Adding categorical priors for active series moves them from fail-safe to normal mode. The existing legislative/policy pattern `(0.05-0.10, 0.55-0.65, 0.25-0.40)` produces rc ∈ [0.22, 0.28] and is the safe template for new event-driven calendar markets.
