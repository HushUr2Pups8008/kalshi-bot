...
def build_batch(**changes: object):
    arguments = {
        ...
        "capture_finished_at": dt("2026-07-12T17:30:05Z"),
        "as_of": dt("2026-07-12T17:30:00Z"),
    }
...
def test_event_retrieval_time_is_part_of_quote_and_snapshot_identity() -> None:
    ...
    changed = build_batch(event=changed_event, capture_finished_at=changed_event.retrieved_at)
...
@pytest.mark.parametrize("failure", ["weather_after_quote", "future_feature", "at_close", "sweep", "target"])
def test_capture_timing_fails_closed(failure: str) -> None:
    batch = build_batch()
    if failure == "weather_after_quote":
