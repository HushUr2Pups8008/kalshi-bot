...
def test_tier_of_path_specific_known_files(tmp_path, monkeypatch):
    ...
    assert tier_of_path(".claude/skills/replay-gate-analyze/SKILL.md") == "T0"
    assert tier_of_path(".claude/ecc-install-plan.json") == "T0"
    assert tier_of_path(".gitignore") == "T0"
    assert tier_of_path("scripts/edge_replay/tier_classifier.py") == "T0"
