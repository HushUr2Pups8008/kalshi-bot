...
**Workoff update (2026-05-15):** F-07 and F-11 P1-B/P1-C landed in
`c982ef5`. Pre-migration snapshot completed at
`mac_archive/db_snapshots/2026-05-15T1407Z`; both snapshot DBs returned
`PRAGMA integrity_check = ok`. Runtime restarted once at
`2026-05-15T14:08:20Z` (new PID `13971`) and post-restart checks verified:
`paper_trades` has `entry_price_cents` and no `market_yes_price`, both
evidence-store tables have `p0_contract_version`, legacy evidence rows are
tagged `0`, `p0_contract: status=ok version=1 row_count=9`, and recent app
logs had no runtime error signatures.
...
| F-07 | High | data | `data/evidence_store.db` + `dossier_updates_post_fix.db` schemas | Neither schema carries `p0_contract_version`, `cohort_id`, or provenance hash | Cohort cut depends entirely on `bot_state.p0_price_fix_deployed_ts` in a different DB file; restore-from-snapshot of evidence_store alone silently mixes cohorts | Replay correctness depends on cross-DB consistency that nothing enforces | **DONE 2026-05-15 in `c982ef5`:** added `p0_contract_version` to `evidence` + `dossier_updates`; legacy rows backfilled to `0`; new writers tag `1`; post-restart live schema verified | `feat/evidence-store-cohort-column` | closed | Claude Code | completed | completed (snapshot + restart) | landed | Bounce 2 completed with pre-migration snapshot + post-restart schema checks | no longer open |
...
| F-11 | Medium | data | `paper_trades.market_yes_price` column-name lie | Documented in `docs/_archive/governance/2026-05-13-market-yes-price-alias-inventory.md` (64-hit inventory as of 2026-05-13) | Any new reader joining on column name without consulting `side` silently mis-prices NO rows | Slow-burn correctness drift; future-author footgun | **DONE:** P1-A landed in `bb9cc95`; P1-B/P1-C landed in `c982ef5`; live DB now has `entry_price_cents` and no `market_yes_price`; offline readers tolerate legacy corpora | `feat/market-yes-price-rename-p1a` | closed | Claude Code | completed | completed (DB schema rename) | landed | Schema rename + replay compatibility completed under Bounce 2 | no longer open |
...
| 7 | **COMPLETE — Evidence-store cohort column** | F-07 | `tasks/evidence_store.py`, `docs/evidence_store_schema.sql`, `docs/evidence_store_schema.md`, tests | Migration + writer tests landed; live schema verified post-restart | completed | additive, completed | n/a | Claude Code | completed | completed | `c982ef5` + Bounce 2 |
| 8 | **COMPLETE — market_yes_price rename (P1-A/B/C)** | F-11 | runtime dataclasses/logs, paper DB DDL/INSERT/hydration, replay/offline readers, tests | Runtime + replay test suites landed; live schema verified post-restart | completed | rename completed | n/a | Claude Code | completed | completed | `bb9cc95` + `c982ef5` |
...
- **Do not** reintroduce `SignalAnalysis.market_yes_price` or `Position.market_yes_price`; P1-A/P1-B removed those runtime aliases and Bounce 2 verified the DB rename. Historical replay readers may still accept legacy `market_yes_price` fields for old corpora, but new runtime surfaces use `executed_price_cents` / `entry_price_cents`.
...
Historical note: the original top-priority workoff prompt below (bothealth P0-awareness) has landed. After Bounce 2, the active follow-up is the F-06 watcher follow-up recorded in `docs/profit_path_debt_log.md`: reconcile the remaining OPPORTUNITY-vs-SKIPPED/PAPER_TRADE accounting gap and determine whether it is an executor branch missing `log_skipped` or watcher double-counting.

Prior prompt retained for audit traceability:
...
Audit artifact: `docs/_archive/governance/2026-05-13-v030x-data-runtime-alignment-audit.md`
...
- Artifact path: `docs/_archive/governance/2026-05-13-v030x-data-runtime-alignment-audit.md` (this file).
