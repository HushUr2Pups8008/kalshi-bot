...
| S4.5c | Extended validation window | IN_PROGRESS | Shared | Provide statistical basis for Section 13 completeness and calibration criteria; final gate before Phase 4 | 72-hour minimum window; production-intended intake; startup probe events excluded; no config changes during window | Section 13 criteria 3, 4, 5, 6 pass against window output; observability completeness review PASS; ≥3 distinct dossier markets observed; signed Section 13 checklist recorded in Notes. **Window opened 2026-04-23 per Contract §10 Rule 6** (runtime-observation task, IN_PROGRESS sub-state in Notes). Window overlaps P2.2 (opened 2026-04-22 per `b86c624`); deliberate parallel run — both windows require the same post-P2.1 (`all_required`) soak with no config changes. 72-hour minimum close: ~2026-04-26. If P2.2 fails and `all_required` is reverted, the S4.5c clock restarts from the revert commit. **Pre-assessment (2026-04-23) against prior-window evidence (2026-04-21 → 2026-04-22):** "Safe to Run in Paper Mode" criterion 3 PASS (47h, zero exceptions), criterion 4 PASS (`BLEND_DECISION ×49`, `DOSSIER_UPDATE ×46`, `EVIDENCE_INGESTION ×46` across 6 distinct markets — already exceeds ≥3 threshold), criterion 5 PASS (zero-baseline rule: 0 paper trades), criterion 6 PASS (vacuous: `CALIBRATION_CHECK` requires market resolution; zero resolutions in window). "Not Done Until" red flags CLEAR: `blend_mode` varies (weighted_blend 67.3% / dominant_lane 32.7%); `trade_blocked_reason` populated on all 49 BLEND_DECISION (all `G1_blended_confidence`, consistent with Phase 0 zero-signal verdict). Traceability chain intact (`evidence_ids_contributing` non-null on all 49). Section 8 required-field audit: 12/16 fields 100% non-null; 4/16 (accumulation/structural p/confidence) are 67.3% non-null — deliberately null in dominant_lane mode per S4.2 accepted interpretation. Final close requires: signed Section 13 checklist + re-run observability completeness review against the new window output. **Post-window sign-off scaffold** (fill at close, earliest 2026-04-26): `[ ]` criterion 3 — runtime=___h, unhandled exceptions=___; `[ ]` criterion 4 — BLEND_DECISION=___, DOSSIER_UPDATE=___, EVIDENCE_INGESTION=___; `[ ]` criterion 5 — trades=___ vs. fast-lane baseline=___ (multiple=___× or zero-baseline N/A); `[ ]` criterion 6 — CALIBRATION_CHECK events=___, max per-lane error vs. baseline=___× (vacuous if zero resolutions); `[ ]` red flag — `blend_mode` variance confirmed (not always `weighted_blend`); `[ ]` red flag — `trade_blocked_reason` populated on all blocked candidates; `[ ]` Section 8 required-field audit — ___/16 fields ≥ 90% non-null; `[ ]` observability completeness review PASS against window output; `[ ]` ≥ 3 distinct dossier markets observed (S4.5b baseline had 6); signed: ______ (Claude | Codex), date: ______. |

**Dependencies:** All Stage 3 tasks COMPLETE before Stage 4 begins. **S4.5a** is complete on test evidence. **S4.5b** COMPLETE (2026-04-23); structural recompute participation verified post-2731d9a via trade-log evidence. **S4.5c** IN_PROGRESS (window opened 2026-04-23, running parallel with P2.2); final integration gate before any Phase 4 (live trading) consideration; pre-assessment against prior-window data shows criteria 3–6 PASS (criterion 6 vacuously). Stage 5 Phases 0–3 may proceed in parallel with S4.5c since they are diagnostics-only.
...
| P2.2 | Run gate in diagnostics mode with tightened override for 3 days | IN_PROGRESS | Shared | Observe `pre_llm_would_block` rate before any enforcement | No enforcement; `diagnostics_only=true` throughout | Gate block rate documented; confirm ≥ 20% before P2.3. Observation window opens with deploy of commit 0f91bf7 (2026-04-22); close at +3 days minimum. |
