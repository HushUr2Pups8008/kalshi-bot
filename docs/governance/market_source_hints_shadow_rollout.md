...
The script scans only `MARKET_SOURCE_HINT_DIAGNOSTIC` records and reports lines scanned, malformed lines skipped, diagnostic/shadow-only counts, non-shadow safety warnings, target/rejection counts, observed modes, hinted sources/domains, tickers, rejected-label reasons, and recent examples. It uses the shared trade-log reader, supports `--since`, `--until`, `--top`, `--recent`, `--exclude-test`, and `--json`, and remains read-only.
