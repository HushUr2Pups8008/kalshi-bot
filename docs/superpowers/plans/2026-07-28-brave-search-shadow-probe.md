...
- Before any fetch, resolve the operator's output-parent alias only to retain the canonical reporting destination, reject canonical ancestor chains writable by another user, then open the canonical parent directory descriptor and `fstat` it for current-effective-UID ownership, directory type, and no group/world write permission. The destination must not be a symlink, directory, or other nonregular path. Keep both staging and directory descriptors open through transport; create, publish, and clean the stage only with directory-FD-relative `os.open`, `os.replace`, and `os.unlink` operations. Before reporting success, re-resolve the textual canonical parent and require its device/inode to match the retained directory descriptor; otherwise fail without returning an output path. Do not use a post-validation stage or destination absolute path.
