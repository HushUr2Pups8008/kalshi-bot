...
- A clean `404` is `SettlementNotFound` only when the exact Polymarket market-by-slug lookup also returns `404`; a market `200` after settlement `404` means nonterminal. Timeout, connection, JSON, status, identity, and schema errors must not become absence.
- Only known Kalshi nonterminal lifecycle statuses return `None`; unknown lifecycle or result shapes raise `SettlementDriftError`.
...
- `None` is reserved for a received HTTP 404. Every other non-2xx response and all malformed payloads propagate as typed errors.

- [ ] **Step 1: Write failing exact-host and 404 tests**

```python
@pytest.mark.asyncio
async def test_kalshi_bounded_exact_market_uses_documented_host_and_maps_only_404_to_none(monkeypatch):
    ...
    assert calls[0]["canonical_host"] == "api.elections.kalshi.com"
...
- `None` is only a received HTTP 404. Both methods require the configured base to be exactly `https://gateway.polymarket.us` and the returned slug to exactly equal the requested slug.
...
@pytest.mark.asyncio
async def test_polymarket_bounded_reader_rejects_wrong_slug_and_non_gateway_host():
...
For Kalshi: require the exact alias/ticker equality, fetch by `venue_market_id`, return `None` for only `_KALSHI_NONTERMINAL`, reject unknown statuses, and normalize terminal markets. For Polymarket: require a numeric canonical `venue_market_id` plus valid slug alias, fetch settlement by slug, then only after a settlement 404 fetch exact market-by-slug with the remaining deadline. Market `200` requires exact slug and canonical id equivalence and returns `None`; market `404` raises `SettlementNotFound`.

To make a correction, first normalize a candidate without a predecessor. If it differs from `prior_observation.observation_sha256`, normalize again with `previous_observation=prior_observation` and `supersedes_observation_sha256=prior_observation.observation_sha256`. If it is identical, return the initial no-supersession object. Do not infer or supply a void refund contract.
